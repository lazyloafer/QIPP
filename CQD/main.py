...
def parse_args(args=None):
    ...
    parser.add_argument('--do_valid', type=bool, default=True, help="do valid")
    parser.add_argument('--do_test', type=bool, default=True, help="do test")

    parser.add_argument('--data_path', type=str, default='data/FB15k-237-betae', help="KG data path")
    ...
    parser.add_argument('-b', '--batch_size', default=2000, type=int, help="batch size of queries")
    ...
    parser.add_argument('--warm_up_steps', default=100000000, type=int, help="no need to set manually, will configure automatically")
    ...
    parser.add_argument('--geo', default='cqd', type=str, choices=['vec', 'box', 'beta', 'cqd'], help='the reasoning model, vec for GQE, box for Query2box, beta for BetaE, cqd for CQD')
    parser.add_argument('--print_on_screen', type=bool, default=True)
    ...
    parser.add_argument('--cqd_t_norm', default=CQD.PROD_NORM, type=str, choices=CQD.NORMS)
    parser.add_argument('--cqd_k', default=5, type=int)
    ...
    parser.add_argument('--use_qa_iterator', type=bool, default=True)
...
def update_args(dataset, args):
    if dataset == 'FB15k-237-betae':
        # args.checkpoint_path = './logs/FB15k-237-betae/1p/cqd/g-cqd/2024.05.10'
        # args.do_train = False
        # args.do_valid = False
        # args.do_test = True
        ...
        args.n = 1
        args.b = 1000
        args.batch_size = args.b
        args.d = 1000
        args.lr = 0.1
        args.warm_up_steps = 10000
        args.max_steps = 450000
        args.cpu_num = 0
        args.geo = 'cqd'
        args.valid_steps = 500
        # args.tasks = '1p'
        epochs = 100
        if args.tasks != '1p': # 0.452783
            if args.tasks == '2p':
                args.cqd_k = 128  # 0.127173
            elif args.tasks == '3p': # 4: 0.072112
                args.cqd_k = 4
                args.cqd_sigmoid = True
                args.cqd_sigmoid_scores = args.cqd_sigmoid
            elif args.tasks == 'pi':
                args.cqd_k = 64  # 0.268309
            elif args.tasks == 'up':
                args.cqd_k = 128  # 16: 0.078406; 64: 0.080028; 128: 0.081315
                args.cqd_sigmoid = True
                args.cqd_sigmoid_scores = args.cqd_sigmoid
            else:
                args.cqd_k = 16 # 2i: 0.357530; 3i: 0.485558; ip: 0.200210; 2u: 0.165978;
        if args.tasks in ['2u', 'up']:
            args.cqd_t_norm = 'min'
            args.cqd_normalize = True
            args.cqd_normalize_scores = args.cqd_normalize
        args.print_on_screen = True
        args.test_batch_size = 1
        args.optimizer = 'adagrad'
        args.reg_weight = 0.05
        args.log_steps = 500
        args.cuda = True
        args.use_qa_iterator = True
    elif dataset == 'NELL-betae':
        # args.checkpoint_path = './logs/NELL-betae/1p/cqd/g-cqd/2024.05.12'
        # args.do_train = False
        # args.do_valid = False
        # args.do_test = True
        args.data_path = 'data/NELL-betae'
        args.n = 1
        args.b = 1000
        args.batch_size = args.b
        args.d = 1000
        ...
        # args.tasks = 'up'
        epochs = 100
        if args.tasks != '1p':  # 0.598
            if args.tasks == '2p':
                args.cqd_k = 128  # 0.214
            elif args.tasks == '3p':  # 4: 0.130870
                args.cqd_k = 4
                args.cqd_sigmoid = True
                args.cqd_sigmoid_scores = args.cqd_sigmoid
            elif args.tasks == 'pi':
                args.cqd_k = 64  # 0.286868
            elif args.tasks == 'up':
                args.cqd_k = 128  # 128: 0.141375
                args.cqd_sigmoid = True
                args.cqd_sigmoid_scores = args.cqd_sigmoid
            else:
                args.cqd_k = 16  # 2i: 0.408671; 3i: 0.498570; ip: 0.253177; 2u: 0.193436;
        if args.tasks in ['2u', 'up']:
            args.cqd_t_norm = 'min'
            args.cqd_normalize = True
            args.cqd_normalize_scores = args.cqd_normalize
        ...
        args.cuda = True
        args.use_qa_iterator = True
    elif dataset == 'FB15k-betae':
        # args.checkpoint_path = './logs/FB15k-betae/1p/cqd/g-cqd/2024.05.13'
        # args.do_train = False
        # args.do_valid = False
        # args.do_test = True
        args.data_path = 'data/FB15k-betae'
        args.n = 1
        args.b = 1000
        args.batch_size = args.b
        args.d = 1000
        ...
        # args.tasks = 'ip'
        epochs = 10
        if args.tasks != '1p':  # 0.8895
            if args.tasks == '2p':
                args.cqd_k = 128  # 0.615
            elif args.tasks == '3p':  # 4: 0.275129
                args.cqd_k = 4
                args.cqd_sigmoid = True
                args.cqd_sigmoid_scores = args.cqd_sigmoid
            elif args.tasks == 'pi':
                args.cqd_k = 64  # 0.641464
            elif args.tasks == 'up':
                args.cqd_k = 128  # 128: 0.141375
                args.cqd_sigmoid = True
                args.cqd_sigmoid_scores = args.cqd_sigmoid
                ...
                args.cqd_k = 16  # 2i: 0.768406; 3i: 0.808385; ip: 0.669905; 2u: 0.734959;
    else:
        raise ValueError('dataset must be in [FB15k-237-betae, FB15k-betae, NELL-betae]')
...
def main(args):
    ...
    if args.do_train:
        steps_per_epoch = len(train_path_iterator.iterator.gi_frame.f_locals['dataloader'].batch_sampler)

        args.warm_up_steps = 1000000000
        args.max_steps = steps_per_epoch * epochs
        args.valid_steps = steps_per_epoch
        args.log_steps = 100
        # print()
