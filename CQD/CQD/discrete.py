...
def score_candidates(s_emb: Tensor,
                     ...
                     queries,
                     scoring_function: Callable[[Tensor, Tensor, Tensor], Tensor]) -> Tuple[Tensor, Optional[Tensor], Optional[Tensor]]:
    ...
    # [B, N]
    atom_scores_2d = scoring_function(s_emb, p_emb, candidates_emb, queries)
...
def query_1p(entity_embeddings: nn.Module,
    ...
    res, _, _ = score_candidates(s_emb=s_emb, p_emb=p_emb,
...
def query_2p(entity_embeddings: nn.Module,
    ...
    # [B, K], [B, K, E]
    atom1_k_scores_2d, x1_k_emb_3d, atom1_k_indices_2d = score_candidates(s_emb=s_emb, p_emb=p1_emb,
    ...
    atom1_k_indices_1d = atom1_k_indices_2d.reshape(-1, 1)
    rel_indices = queries[:, 1]
    n_copies = atom1_k_indices_1d.size(0) // batch_size
    rel_indices = rel_indices.reshape(-1, 1, 1).repeat(1, n_copies, 1).reshape(-1, 1)
    queries = torch.concat([atom1_k_indices_1d, rel_indices], dim=-1)
    # [B * K, N]
    atom2_scores_2d, _, _ = score_candidates(s_emb=x1_k_emb_2d, p_emb=p2_emb,
                                             candidates_emb=candidates_emb, k=None,
                                             entity_embeddings=entity_embeddings,
                                             queries=queries,
                                             scoring_function=scoring_function)
...
def query_3p(entity_embeddings: nn.Module,
    ...
    n_copies = atom1_k_indices_1d.size(0) // batch_size
    rel_indices = rel_indices.reshape(-1, 1, 1).repeat(1, n_copies, 1).reshape(-1, 1)
    sub_queries = torch.concat([atom1_k_indices_1d, rel_indices], dim=-1)

    # [B * K, K], [B * K, K, E]
    atom2_k_scores_2d, x2_k_emb_3d, atom2_k_indices_2d = score_candidates(s_emb=x1_k_emb_2d, p_emb=p2_emb,
                                                                          candidates_emb=candidates_emb, k=k,
                                                                          entity_embeddings=entity_embeddings,
                                                                          queries=sub_queries,
                                                                          scoring_function=scoring_function)
    ...
    atom2_k_indices_1d = atom2_k_indices_2d.reshape(-1, 1)
    rel_indices = queries[:, 2]
    n_copies = atom2_k_indices_1d.size(0) // batch_size
    rel_indices = rel_indices.reshape(-1, 1, 1).repeat(1, n_copies, 1).reshape(-1, 1)
    sub_queries = torch.concat([atom2_k_indices_1d, rel_indices], dim=-1)

    # [B * K * K, N]
    atom3_scores_2d, _, _ = score_candidates(s_emb=x2_k_emb_2d, p_emb=p3_emb,
                                             candidates_emb=candidates_emb, k=None,
                                             entity_embeddings=entity_embeddings,
                                             queries=sub_queries,
                                             scoring_function=scoring_function)
...
def query_ip(entity_embeddings: nn.Module,
    ...
    scores_1_k_indices_1d = scores_1_k_indices.reshape(-1, 1)
    rel_indices = queries[:, 4]
    n_copies = scores_1_k_indices_1d.size(0) // batch_size
    rel_indices = rel_indices.reshape(-1, 1, 1).repeat(1, n_copies, 1).reshape(-1, 1)
    queries = torch.concat([scores_1_k_indices_1d, rel_indices], dim=-1)
    # [B * K, N]
    scores_2, _, _ = score_candidates(s_emb=scores_1_k_emb_2d, p_emb=p_emb, candidates_emb=e_emb, k=None,
                                      entity_embeddings=entity_embeddings,
                                      queries=queries,
                                      scoring_function=scoring_function)
