...
def load_kbc(model_path, device, nentity, nrelation, opt):
    model = ComplEx(sizes=[nentity, nrelation, nentity], rank=1000, init_size=1e-3, opt=opt)
...
def neural_adj_matrix(model, rel, nentity, device, thrshd, adj_list):
    ...
    for (h, t) in adj_list:  # each h connects how many rel i
        num[h, 0] += 1
    num = torch.maximum(num, torch.ones(nentity, 1).to(torch.float).to(device))  # if h doesn't connect rel i, set 1 forcibly
    for s in range(0, nentity, bsz):  # Judge the conversion probability of all entities under rel
        t = min(nentity, s+bsz)
...
class KGReasoning(nn.Module):
    def __init__(self, args, device, adj_list, query_name_dict, name_answer_dict, opt):
        ...
        if args.data_path.split('/')[1].split('-')[1] == "237":
            dataset_name += "-237"
        filename = 'neural_adj_plm/'+dataset_name+'_'+str(args.fraction)+'_'+str(args.thrshd)+'.pt'
        if os.path.exists(filename):
            self.relation_embeddings = torch.load(filename, map_location=device)
        else:
            if not os.path.exists('neural_adj_plm/{}/'.format(dataset_name)):
                os.mkdir('neural_adj_plm/{}/'.format(dataset_name))
            kbc_model = load_kbc(args.kbc_path, device, args.nentity, args.nrelation, opt)
            for i in tqdm(range(args.nrelation)):
                self.relation_embeddings = list()
                chunk_filename = 'neural_adj_plm/{}/'.format(dataset_name) + dataset_name + '_' + str(args.fraction) + '_' + str(args.thrshd) + '_{}.pt'.format(str(i))
                ...
                torch.save(self.relation_embeddings, chunk_filename)
